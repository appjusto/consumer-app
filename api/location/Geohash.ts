...
class Geohash {
   ...
   */
  static bounds(_geohash: string): {
    sw: { lat: number; lng: number };
    ne: { lat: number; lng: number };
  } {
    if (_geohash.length === 0) throw new Error('Invalid geohash');

    const geohash = _geohash.toLowerCase();
   ...
   */
  static adjacent(_geohash: string, direction: 'n' | 's' | 'e' | 'w'): string {
